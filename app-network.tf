...
resource "azurerm_network_security_group" "app_nsg" {
  ...
  dynamic security_rule {
    for_each = local.networksercuritygroup_rules
    content{
...
resource "azurerm_subnet_network_security_group_association" "subnet_appnsg" {
  network_security_group_id = azurerm_subnet.app_network_subnets["dbsubnet01"].id
  subnet_id                 = azurerm_network_security_group.app_nsg.id
}